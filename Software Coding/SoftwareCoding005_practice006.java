...
public class SoftwareCoding005_practice006 {
	...
	public static void main(String[] args) {
		...
		for (int k34_i = 0; k34_i < k34_itemName.length; k34_i++) {// k34_i는 0부터 시작하고, k34_itemName 배열의 길이만큼 반복하며, 실행문이 끝나면 k34_i를 1씩 증가시킨다.
																   // 총 지불 금액을 구하기 위한 반복문
			if (k34_taxfree[k34_i]) {// 만약 k34_taxfree 배열에 k34_i인덱스의 값이 참이라면(면세 상품이라면)
				k34_taxFreeSum += k34_price[k34_i] * k34_amount[k34_i];// k34_taxFreeSum에 가격 * 수량을 곱한 값을 누적 합산한다.
			} else {// k34_taxfree 배열에 k34_i인덱스의 값이 거짓이라면(과세 상품이라면)
		...
		System.out.printf("\n");
		System.out.printf("%22s%14.14s\n", "총 품목 수량", k34_df.format(k34_itemName.length));
	...
	public static void k34_printItemName(String k34_s) {// 아이템 이름의 길이를 세기 위한 함수
		...
		for (int k34_i = 0; k34_i < k34_str.length; k34_i++) {// 아이템 이름의 길이를 세기 위한 반복문
			if (k34_str[k34_i] >= 'A' && k34_str[k34_i] <= 'z') {// k34_str배열 k34_i번째 인덱스의 값이 영어이면
				...
				k34_byte++;// 그 외 띄어쓰기나 특수문자라면 1을 더한다.      14 영어면 = 그냥 출력하면 14 한글이면 = 이거 출력하면ㅇ ㅏㄴ되고 대신
			}
