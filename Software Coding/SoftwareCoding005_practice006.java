...
public class SoftwareCoding005_practice006 {
	...
	public static void main(String[] args) {
		String[] k34_itemName = { "초코파이", "바나나우유", "건포도", "오렌지주스", "초코에몽", "피코크 들깨미역국500g", "남양 맛있는우유 GT 저지방", "녹차원 흑당시럽 10입",
				"진라면소컵(매)65*6", "케라시스 앰플 트리트", "푸르밀 미숫가루 우유", "자일로스갈색설탕500g", "CJ 비비고칩 20G", "피코크 들깨미역국 500g", "흑원당 말차 밀크티",
		...
		int k34_totalSum = 0;// 과세 상품 가격의 합을 구하기 위해 int변수 k34_totalSum을 선언하고 0으로 초기화한다.
		int k34_taxFreeSum = 0;// 면세 상품 가격의 합을 구하기 위해 int변수 k34_taxFreeSum을 선언하고 0으로 초기화한다.
		
		// 과세 상품과 면세 상품의 합계 금액 구하기
		for (int k34_i = 0; k34_i < k34_itemName.length; k34_i++) {// k34_i는 0부터 시작하고, k34_itemName 배열의 길이만큼 반복하며, 실행문이 끝나면 k34_i를 1씩 증가시킨다.
																   // 총 지불 금액을 구하기 위한 반복문
			...
			} else {// k34_taxfree 배열에 k34_i인덱스의 값이 거짓이라면
				k34_totalSum += k34_price[k34_i] * k34_amount[k34_i];// k34_totalSum에 가격 * 수량을 곱한 값을 누적 합산한다.
			}
		}
		
		int k34_netPrice = (int) (k34_totalSum / (1 + k34_taxRate));// 세전 금액 계산을 위한 int변수 k34_netPrice를 선언하고 과세 대상 금액을 (1 + 세율)로 나눈 값으로 초기화한다.
		double k34_taxPrice = k34_netPrice * k34_taxRate;// 부가세를 계산하기 위한 int변수 k34_taxPrice를 선언하고 세전 금액을 세율로 곱한 값으로 저장한다.
		int k34_totalTaxPrice = 0;// 부가세 올림처리 계산을 위한 int변수 k34_totalTaxPrice를 선언하고 0으로 초기화한다.
		...
		SimpleDateFormat k34_sd1 = new SimpleDateFormat("YYYY-MM-dd HH:mm");// 현재 시간을 양식대로 출력하기 위해 SimpleDateFormat클래스의 객체를 생성하고 양식을 지정한다.
		...
		for (int k34_i = 0; k34_i < k34_itemName.length; k34_i++) {// k34_i는 0부터 시작하고, k34_i가 k34_itemName.length 배열의 길이보다 작으면 반복되며, 실행문이 끝나면 k34_i를 1씩 증가시킨다.
			if (k34_itemName[k34_i].length()< 13) {// 만약 k34_itemName 배열의 k34_i번째 인덱스에 저장된 품목명의 길이가 13 미만이라면
				k34_itemNamebyte = k34_getTextByte(k34_itemName[k34_i]);// k34_getTextByte함수를 호출하고 인자로 품목명을 그대로 넣는다.
			} else {// k34_itemName 배열의 k34_i번째 인덱스에 저장된 품목명의 길이가 13 이상이라면
				k34_itemNamebyte = k34_getTextByte(k34_itemName[k34_i].substring(0, 12));// k34_getTextByte함수를 호출하고 substring을 이용해 1 ~ 12번째까지 글자를 자른 후 넣는다.
			}
			...
			if (k34_taxfree[k34_i]) {// 면세 대상 품목에 별을 추가하기 위한 조건문
									 // k34_taxfree 배열의 k34_i번째 인덱스의 값이 참이라면
				System.out.printf("* ");// 별을 출력한다.
			} else {// k34_taxfree 배열의 k34_i번째 인덱스의 값이 거짓이라면
				System.out.printf("  ");
			}
			System.out.printf("%-1.12s", k34_itemName[k34_i]);// 품목명을 출력한다.
				for (int k34_j = k34_itemNamebyte; k34_j < 24; k34_j++) {// 문자열의 길이를 맞추기 위한 반복문
					System.out.printf(" ");
				}
		...
		System.out.printf("\n");
		System.out.printf("               총 품목 수량           %8.8s\n", k34_df.format(k34_itemName.length));
		System.out.printf("            (*)면 세  물 품           %8.8s\n", k34_df.format(k34_taxFreeSum));
		System.out.printf("               과 세  물 품           %8.8s\n", k34_df.format(k34_netPrice));
		System.out.printf("               부   가   세           %8.8s\n", k34_df.format(k34_totalTaxPrice));
		System.out.printf("               합        계           %8.8s\n", k34_df.format(k34_totalPrice));
		System.out.printf("결 제 대 상 금 액                     %8.8s\n", k34_df.format(k34_totalPrice));
		System.out.printf("----------------------------------------------\n");
