...
public class SoftwareCoding006_practice013_InputDataMain {
	...
	public static void main(String[] args) {
		...
		int k34_person = 12;// 이 값을 주는 만큼 배열이 생성된다.
		
		// 메소드 호출을 위해 SoftwareCoding006_practice013_InputDataClass 클래스의 객체를 생성한다.
		SoftwareCoding006_practice013_InputDataClass k34_data = new SoftwareCoding006_practice013_InputDataClass(k34_person);
		
		for (int k34_i = 0; k34_i < k34_person; k34_i++) {// InputData 클래스의 인스턴스에 값을 저장하기 위한 반복문
			...
			k34_data.k34_setData(k34_i, k34_name, k34_kor, k34_eng, k34_mat);// k34_setData 메소드를 호출해 값을 저장한다.
		}
		...
		for (int k34_i = 0; k34_i < k34_person; k34_i++) {// 입력 수 만큼 목록을 출력하기 위한 반복문
			...
			k34_sum[0] += k34_data.k34_kor[k34_i];// 현재 페이지 국어 점수의 합계를 알기 위해 국어 점수를 더한다.
			k34_sum[5] += k34_data.k34_kor[k34_i];// 누적 페이지 국어 점수의 합계를 알기 위해 국어 점수를 더한다.
			k34_sum[1] += k34_data.k34_eng[k34_i];// 현재 페이지 영어 점수의 합계를 알기 위해 영어 점수를 더한다.
			k34_sum[6] += k34_data.k34_eng[k34_i];// 누적 페이지 영어 점수의 합계를 알기 위해 영어 점수를 더한다.
			k34_sum[2] += k34_data.k34_mat[k34_i];// 현재 페이지 수학 점수의 합계를 알기 위해 수학 점수를 더한다.
			k34_sum[7] += k34_data.k34_mat[k34_i];// 누적 페이지 수학 점수의 합계를 알기 위해 수학 점수를 더한다.
			k34_sum[3] += k34_data.k34_sum[k34_i];// 현재 페이지 총점의 합계를 알기 위해 총점을 더한다.
			k34_sum[8] += k34_data.k34_sum[k34_i];// 누적 페이지 총점의 합계를 알기 위해 총점을 더한다.
			k34_sum[4] += k34_data.k34_avg[k34_i];// 현재 페이지 평균의 합계를 알기 위해 평균을 더한다.
			k34_sum[9] += k34_data.k34_avg[k34_i];// 누적 페이지 평균의 합계를 알기 위해 평균을 더한다.
